...
| Simulator blank / stream `/camera/stream` tidak jalan tanpa hardware | `samples/conveyor.mp4` hilang atau `ICAM_SIM_SOURCE` salah | File sudah tersedia di `samples/conveyor.mp4` (1280×720, 24fps). Regenerate dgn ffmpeg: `ffmpeg -f lavfi -i testsrc=duration=10:size=1280x720:rate=24 -c:v libx264 -pix_fmt yuv420p samples/conveyor.mp4`. Alternatif: kosongkan `ICAM_SIM_SOURCE` → ml-service pakai synthetic frame |
| Callback ml-service tidak sampai / URL mismatch | `APP_URL` Laravel ≠ `LARAVEL_URL` ml-service (`localhost` vs `127.0.0.1`) | Samakan keduanya. Repo ini dipatok ke `http://127.0.0.1:8000` di kedua `.env`; setelah ubah `.env`, jalankan `php artisan config:clear` |
